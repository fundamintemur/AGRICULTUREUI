    ...
    public class ReportController : Controller
    {
        ...
        public IActionResult ContactReport()
        {
            using (var workBook = new XLWorkbook())
            {
                ...
                {
                    //workbook stream kaydettik.
                    workBook.SaveAs(stream);
                    //stream toarray dönüştürdük.
        ...
        public IActionResult AnnouncementReport()
        {
            using (var workBook = new XLWorkbook())
            {
                ...
                using (var stream = new MemoryStream())
                {
                    ...
                    string excel_deger;
                    excel_deger = g1.ToString();
                    workBook.SaveAs(stream);
                    var content = stream.ToArray();
                    return File(content, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", excel_deger + ".xlsx");
                }
            }
        }
    }
